...
def compare_all_different(list1, list2):
    ...
    for dict1 in list1:
        ...
        report_year = dict1['报告年份']  # 报告年份
        # derive_year_enterprise_carbon_market_emissions = dict1['排放量(tCO2)']  # 排放量(tCO2)
        # 通过一个字段标志另一个列表中的唯一字典
        dict2 = get_dict_wih_same_key(new_list1=[enterprise_name, report_year], list2=list2)
        # 接下来就是两条字典数据比对
        try:
...
def get_dict_wih_same_key(new_list1, list2):
    for dict2 in list2:
        if dict2['企业名称'] == new_list1[0] and dict2['报告年份'] == new_list1[1]:
            return dict2
...
if __name__ == '__main__':
    my_excel_file_path = '../../../resource/carbon_market.xlsx'
    # my_sheet_name = '排放量信息'
    my_sheet_name = '基本信息'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_carbon_market_enterprise.json'
