...
def compare_all_different(list1, list2):
    ...
    if count(list1) != count(list2):
        ...
        for dict1 in list1:
            stock_code = dict1['证券代码']  # 证券代码
            enterprise_abbreviation = dict1['公司简称']  # 公司简称
            # 通过一个字段标志另一个列表中的唯一字典
            dict2 = get_dict_wih_same_key(new_list1=[stock_code, enterprise_abbreviation], list2=list2)
            # 接下来就是两条字典数据比对
            try:
...
def get_dict_wih_same_key(new_list1, list2):
    for dict2 in list2:
        dict2['证券代码'] = dict2['证券代码'].split('.')[0]
        if dict2['公司简称'] == new_list1[1]:
            return dict2
...
if __name__ == '__main__':
    ...
    my_json_file_path = '../../../resource/ads_enterprise_tcfd.json'
