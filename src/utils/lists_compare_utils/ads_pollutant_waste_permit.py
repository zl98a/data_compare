...
def compare_all_different(list1, list2):
    ...
    for dict1 in list1:
        permit_no = dict1['排污许可证编号']  # 排污许可证编号
        enterprise_name = dict1['企业名称']  # 企业名称
        # 通过一个字段标志另一个列表中的唯一字典
        dict2 = get_dict_wih_same_key(new_list1=[permit_no, enterprise_name], list2=list2)
        # 接下来就是两条字典数据比对
        try:
...
def get_dict_wih_same_key(new_list1, list2):
    i = 0
    for dict2 in list2:
        if dict2['排污许可证编号'] == new_list1[0] and dict2['企业名称'] == new_list1[1]:
            return dict2
...
if __name__ == '__main__':
    my_excel_file_path = '../../../resource/pollutant_waste_permit.xlsx'
    my_sheet_name = '废气&废水'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_pollutant_waste_permit.json'
