...
def compare_all_different(list1, list2):
    ...
    if count(list1) != count(list2):
        ...
        for dict1 in list1:
            ...
            derive_year_enterprise_boundary_pollutant_emission = dict1['分边界污染物排放量']  # 分边界污染物排放量
            # 通过三个字段标志另一个列表中的唯一字典
            dict2 = get_dict_wih_same_key(new_list1=[year, enterprise_name, boundary, pollution, derive_year_enterprise_boundary_pollutant_emission], list2=list2)
            # 接下来就是两条字典数据比对
            try:
