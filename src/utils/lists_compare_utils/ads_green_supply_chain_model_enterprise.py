...
def compare_all_different(list1, list2):
    ...
    for dict1 in list1:
        batch = dict1['批次']  # 批次
        region = dict1['地区']  # 地区
        plant_name = dict1['企业名称']  # 企业名称
        # 通过一个字段标志另一个列表中的唯一字典
        dict2 = get_dict_wih_same_key(new_list1=[batch, region, plant_name], list2=list2)
        # 接下来就是两条字典数据比对
        try:
...
def get_dict_wih_same_key(new_list1, list2):
    for dict2 in list2:
        if dict2['批次'] == new_list1[0] and dict2['地区'] == new_list1[1] and dict2['企业名称'] == new_list1[2]:
            return dict2
...
if __name__ == '__main__':
    ...
    my_sheet_name = '绿色供应链管理示范企业'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_green_supply_chain_model_enterprise.json'
