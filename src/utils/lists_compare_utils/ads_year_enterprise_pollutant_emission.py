...
def compare_all_different(list1, list2):
    ...
    for dict1 in list1:
        ...
        # 通过一个字段标志另一个列表中的唯一字典
        dict2 = get_dict_wih_same_key(new_list1=[year, enterprise_name, pollution], list2=list2)
        # 接下来就是两条字典数据比对
        try:
...
def get_dict_wih_same_key(new_list1, list2):
    i = 0
    for dict2 in list2:
        if dict2['年份'] == new_list1[0] and dict2['企业名称'] == new_list1[1] and dict2['污染物'] == new_list1[2]:
            return dict2
...
if __name__ == '__main__':
    ...
    my_sheet_name = '企业污染物排放'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_year_enterprise_pollutant_emission.json'
