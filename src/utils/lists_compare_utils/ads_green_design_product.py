...
def compare_all_different(list1, list2):
    ...
    for dict1 in list1:
        batch = dict1['批次']  # 批次
        product_type = dict1['产品类别']  # 产品类别
        enterprise_name = dict1['企业名称']  # 企业名称
        product_name = dict1['产品名称']  # 产品名称
        product_model = dict1['产品型号'] # 产品型号
        # 通过一个字段标志另一个列表中的唯一字典
        dict2 = get_dict_wih_same_key(new_list1=[batch, product_type, enterprise_name, product_name, product_model], list2=list2)
        # 接下来就是两条字典数据比对
        try:
...
def get_dict_wih_same_key(new_list1, list2):
    for dict2 in list2:
        if dict2['批次'] == new_list1[0] and dict2['产品类别'] == new_list1[1] and dict2['企业名称'] == new_list1[2]\
                and dict2['产品名称'] == new_list1[3] and dict2['产品型号'] == new_list1[4]:
            return dict2
...
if __name__ == '__main__':
    my_excel_file_path = '../../../resource/green_evaluation.xlsx'
    my_sheet_name = '绿色设计产品'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_green_design_product.json'
