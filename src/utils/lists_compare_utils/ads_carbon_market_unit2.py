...
def compare_all_different(list1, list2):
    ...
    for dict1 in list1:
        ...
        year = dict1['年份']  # 年份
        confirm_method = dict1['确定方式']  # 确定方式
        unit_name = dict1['机组']  # 机组
        month = dict1['月份']  # 月份
        # 通过一个字段标志另一个列表中的唯一字典
        dict2 = get_dict_wih_same_key(new_list1=[enterprise_name, year, confirm_method, unit_name, month], list2=list2)
        # 接下来就是两条字典数据比对
        try:
...
def get_dict_wih_same_key(new_list1, list2):
    i = 0
    for dict2 in list2:
        if dict2['企业名称'] == new_list1[0] and dict2['年份'] == new_list1[1] and dict2['确定方式'] == new_list1[2]\
                and dict2['机组'] == new_list1[3] and dict2['月份'] == new_list1[4]:
                return dict2
...
if __name__ == '__main__':
    ...
    my_sheet_name = '委托检测关键词'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_carbon_market_unit2.json'
