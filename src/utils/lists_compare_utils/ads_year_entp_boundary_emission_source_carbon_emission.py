...
def compare_all_different(list1, list2):
    ...
    if count(list1) != count(list2):
        ...
        for dict1 in list1:
            ...
            emission_source = dict1['排放源']  # 排放源
            carbon_emission = dict1['分边界分排放源碳排放量（t）']  # 分边界分排放源碳排放量（t）
            # 通过三个字段标志另一个列表中的唯一字典
            dict2 = get_dict_wih_same_key(new_list1=[year, enterprise_name, boundary, emission_source, carbon_emission], list2=list2)
            # 接下来就是两条字典数据比对
            try:
...
def get_dict_wih_same_key(new_list1, list2):
    ...
    for dict2 in list2:
        if dict2['年份'] == new_list1[0] and dict2['企业名称'] == new_list1[1] and dict2['边界'] == new_list1[2]\
                and dict2['排放源'] == new_list1[3] and dict2['分边界分排放源碳排放量（t）'] == new_list1[4]:
            return dict2
...
if __name__ == '__main__':
    ...
    my_sheet_name = '分边界分排放源碳排放'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_year_entp_boundary_emission_source_carbon_emission.json'
